(function($) {
	...
	window.MNKYTestimonialsView = vc.shortcode_view.extend({
		...
		render:function () {
			window.MNKYTestimonialsView.__super__.render.call(this);
		...
		changeShortcodeParams:function (model) {
			window.MNKYTestimonialsView.__super__.changeShortcodeParams.call(this, model);
		...
		_loadDefaults:function () {
			window.MNKYTestimonialsView.__super__._loadDefaults.call(this);
		}
	});
	
	
	window.MNKYContentSliderView = vc.shortcode_view.extend({
		adding_new_tab:false,
		events:{
			'click .add_tab':'addTab',
			'click > .vc_controls .column_delete, > .vc_controls .vc_control-btn-prepend':'addTab',
			'click > .vc_controls .column_delete, > .vc_controls .vc_control-btn-delete':'deleteShortcode',
			'click > .vc_controls .column_edit, > .vc_controls .vc_control-btn-edit':'editElement',
			'click > .vc_controls .column_clone,> .vc_controls .vc_control-btn-clone':'clone'
		},
		render: function() {
			return window.MNKYContentSliderView.__super__.render.call(this), vc_user_access().shortcodeAll("vc_accordion_tab") ? (this.$content.sortable({
				axis: "y",
				handle: ".wpb_element_wrapper,.vc_element-move",
				stop: function(event, ui) {
		...
		addTab: function(e) {
				return e.preventDefault(), vc_user_access().shortcodeAll("mnky_content_slider_tab") ? (this.adding_new_tab = !0, void vc.shortcodes.create({
					shortcode: "mnky_content_slider_tab",
					params: {},
					parent_id: this.model.id
				})) : !1
			},
			_loadDefaults: function() {
				window.MNKYContentSliderView.__super__._loadDefaults.call(this)
			}
	
	});
	
	
	window.MNKYContentSliderTabView = window.VcColumnView.extend({
		events:{
		...
		changeShortcodeParams:function (model) {
			var params = model.get('params');
			window.MNKYContentSliderTabView.__super__.changeShortcodeParams.call(this, model);
			if (_.isObject(params)) {
				this.$el.find( '.wpb_column_container' ).before( this.$el.find( 'h3.title' ) );
			}			
		}
	});	
	
	
	window.MNKYPricingView = window.VcColumnView.extend({
		events:{
		  'click > .vc_controls .vc_control-btn-delete':'deleteShortcode',
		  'click > .vc_controls .vc_control-btn-prepend':'addElement',
		  'click > .vc_controls .vc_control-btn-edit':'editElement',
		...
		changeShortcodeParams:function (model) {
			var params = model.get('params');
			window.MNKYPricingView.__super__.changeShortcodeParams.call(this, model);
			if (_.isObject(params)) {
	...
	window.MNKYContentBoxView = window.VcColumnView.extend({
		events:{
		  'click > .vc_controls .vc_control-btn-delete':'deleteShortcode',
		  'click > .vc_controls .vc_control-btn-prepend':'addElement',
		  'click > .vc_controls .vc_control-btn-edit':'editElement',
