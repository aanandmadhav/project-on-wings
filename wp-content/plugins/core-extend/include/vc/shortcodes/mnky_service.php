<?php
$output = $heading_style = $text_style = $link = $a_href = $a_title = $a_target = $a_rel = '';
	...
	if ( $use_link ) {
		...
		$output .= wpb_js_remove_wpautop($content);
		$output .= '</div>';
		$output = '<div class="clearfix mnky_service-box'. esc_attr( $el_class ) . esc_attr( $position ) .'">' . $output . '</a></div>';
	}
