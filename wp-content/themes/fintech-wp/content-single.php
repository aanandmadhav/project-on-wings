...
?>
	...
	<link itemprop="mainEntityOfPage" href="<?php the_permalink(); ?>" />
		...
		<?php if( ot_get_option('post_date') == 'off' || get_post_meta( get_the_ID(), 'mnky_single_post_header', true ) == 'off' ) : ?>
		...
		<?php if( ot_get_option('post_author') == 'off' || get_post_meta( get_the_ID(), 'mnky_single_post_header', true ) == 'off' ) : ?>
	...
	<?php if ( get_the_author_meta( 'description' ) && ot_get_option('author_description') != 'off' ) : ?>
		...
		<?php echo get_avatar( get_the_author_meta( 'ID' ), 100); ?>
			<div class="fn">
				<?php echo '<a class="url" href="'. esc_url(get_author_posts_url( get_the_author_meta( 'ID' ) )) .'" title="'. esc_attr(sprintf( __( 'View all posts by %s', 'fintech-wp' ), get_the_author() )) .'" rel="author">'. get_the_author() .'</a>'; ?>
			</div><!-- .fn -->	
