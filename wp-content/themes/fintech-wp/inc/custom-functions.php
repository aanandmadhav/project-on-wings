...
if ( ! function_exists( 'mnky_post_time' )) {
	function mnky_post_time() {
		...
		$mnkytime = time() - $time;
		if($mnkytime < 60){
			$mnkytimestamp = __('Just now', 'fintech-wp');
		}elseif($mnkytime < 604800 ){
			$mnkytimestamp = sprintf(__('%s ago', 'fintech-wp'), human_time_diff($time));
		}else{
			$mnkytimestamp = sprintf( __('%1$s %2$s', 'fintech-wp'), get_the_date(), get_the_time()); 
		}
	...
	function mnky_blog_meta() {
		...
		echo '<div class="entry-meta-blog">';
			...
			if ( ot_get_option('post_author_blog') != 'off' ) {
				echo '<a class="meta-author url" href="'. esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) .'" title="'. esc_attr(sprintf( __( 'View all posts by %s', 'fintech-wp' ), get_the_author() )) .'" rel="author"><span itemprop="author" itemscope itemtype="http://schema.org/Person"><span itemprop="name">'.  get_the_author() .'</span></span></a>';
			}
	
			if ( ot_get_option('post_date_blog') != 'off' ) {
				echo '<a href="'. esc_url(get_permalink()) .'" title="'. sprintf( esc_attr__( 'View %s', 'fintech-wp' ), get_the_title() ) .'"><span class="meta-date"><time class="published" datetime="'. esc_attr(get_the_date( 'c' )) .'" itemprop="datePublished">'. esc_html( trim( mnky_post_time() ) ) .'</time><time class="meta-date-modified updated" datetime="'. esc_attr(get_the_modified_date( 'c' )) .'" itemprop="dateModified">'. esc_attr(get_the_modified_date()) .'</time></span></a>';
			}
				
			if ( ot_get_option('post_comments_blog') != 'off' ) {
				if( comments_open() && get_comments_number() == 1 ){
					echo '<span class="meta-comments"><a href="'. esc_url(get_comments_link()) .'" title="'. esc_html__('Comments', 'fintech-wp') .'">'. sprintf( esc_html__( '%s Comment', 'fintech-wp' ), get_comments_number() ) .'</a></span><meta itemprop="interactionCount" content="UserComments:'. esc_html(get_comments_number()) .'"/>';
				} elseif ( comments_open()) {
					echo '<span class="meta-comments"><a href="'. esc_url(get_comments_link()) .'" title="'. esc_html__('Comments', 'fintech-wp') .'">'. sprintf( esc_html__( '%s Comments', 'fintech-wp' ), get_comments_number() ) .'</a></span><meta itemprop="interactionCount" content="UserComments:'. esc_html(get_comments_number()) .'"/>';
				}
			}
...
if ( ! function_exists( 'mnky_post_links' ) ) {
	function mnky_post_links() {
		if( is_single() && ot_get_option('post_links') != 'off' ){
			...
			previous_post_link('<span class="previous-post-link"><span class="previous-post-title">'. esc_html__( 'Previous Article', 'fintech-wp' ) .'</span>%link</span>'); 
			next_post_link('<span class="next-post-link"><span class="previous-post-title">'. esc_html__( 'Next Article', 'fintech-wp' ) .'</span>%link</span>');
			echo '</div>';
		}
	}
}
