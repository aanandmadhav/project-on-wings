			...
			<header class="page-header clearfix">	
				<h1>
					<?php if ( is_front_page() ) {
						echo esc_html__( 'Blog', 'fintech-wp');
					} else {
					single_post_title();
					} ?>
				</h1>	
		...
		<header class="page-header clearfix">
			<h1>
				...
				elseif ( is_search() ) : // Search title
					printf( esc_html__( 'Search Results for: %s', 'fintech-wp' ), '<span>' . get_search_query() . '</span>' );
						
				elseif ( is_404() ) :
					echo esc_html__( 'Oops, page not found', 'fintech-wp');
